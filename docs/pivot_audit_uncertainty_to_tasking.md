...
- **Slice 17** — local decision API contract (`custody.api.schemas`, `custody.api.service`, `custody.api.app` stub, seven endpoints covering health / decision-packet / collect ranking / optimize plan / evaluate policies / planner queue / portfolio allocation, `scripts/21_api_demo.py`). Pending commit hash on this slice.
