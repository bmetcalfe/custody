...
- **Slice 19** — real-artifact evidence bridge (`ArtifactRecord`, `ArtifactEvidenceBundle`, `ArtifactDecisionBundle`, explicit semantic and scenario signal mapping paths, committed fixture manifests under `tests/fixtures/artifacts/`, `scripts/23_packet_from_artifacts.py`). Pending commit hash on this slice.
