...
- **Slice 22** — collection-window scheduler-lite (`CollectionWindow`, `ScheduleConstraint`, `ScheduledCollect`, `SchedulePlan`, `ScheduleReport`, deterministic exhaustive search over (candidate, window) pairs under capacity / timing / overlap / resource-type / exclusion constraints, committed window fixtures under `tests/fixtures/schedule/`, `scripts/26_schedule_collect_windows.py`). Pending commit hash on this slice.
