...
The "New components required" table above was the Week 0 plan. Slices 1–12 of that plan have landed. Each slice is one focused commit on `main`:
...
- **Slice 12** — heuristic collection-policy evaluator (`evaluate_collection_policies`, six named deterministic policies, RL-ready evaluation substrate, `scripts/16_evaluate_collect_policies.py`). Pending commit hash on this slice.
