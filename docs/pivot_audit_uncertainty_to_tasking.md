...
- **Slice 16** — portfolio-level collection allocation (`PortfolioConstraint`, `PortfolioCandidate`, `PortfolioAllocationReport`, exhaustive + greedy cross-scenario optimization under budget / max-collect / per-scenario constraints, `scripts/20_portfolio_allocation.py`). Landed in `54a5487`.
- **Slice 17** — local decision API contract (`custody.api.schemas`, `custody.api.service`, `custody.api.app` stub, seven endpoints covering health / decision-packet / collect ranking / optimize plan / evaluate policies / planner queue / portfolio allocation, `scripts/21_api_demo.py`). Landed in `858f0a0`.
- **Slice 18** — provenance & DevSecOps posture (`custody.provenance` with `ProvenanceRecord` / `InputReference` / `build_provenance_record` / `stable_run_id` / `get_git_commit` / `attach_provenance` / text/JSON/markdown formatters; `scripts/22_provenance_manifest.py` standalone manifest CLI; provenance attached to every API response; `docs/security.md` and `docs/devsecops.md` documenting the prototype security posture; `.github/workflows/tests.yml` adding the first CI workflow). Landed in `048de25`.
- **Slice 19** — real-artifact evidence bridge (`ArtifactRecord`, `ArtifactEvidenceBundle`, `ArtifactDecisionBundle`, explicit semantic and scenario signal mapping paths, committed fixture manifests under `tests/fixtures/artifacts/`, `scripts/23_packet_from_artifacts.py`). Landed in `9880375`.
- **Slice 20** — scene-availability metadata bridge (`SceneAvailabilityRecord`, `SceneAvailabilityCatalog`, `CollectFeasibilityAssessment`, `AvailabilityAdjustedRecommendation`, deterministic feasibility rules over provider-neutral SAR / optical / AIS metadata, committed metadata fixtures under `tests/fixtures/availability/`, `scripts/24_scene_availability.py`). Landed in `228a357`.
- **Slice 21** — availability-adjusted collection optimization (`AvailabilityOptimizerConstraint`, `AvailabilityAdjustedPlanItem`, `AvailabilityOptimizedPlan`, `AvailabilityOptimizationReport`, exhaustive + greedy baselines with metadata-adjusted utility, `scripts/25_availability_optimized_plan.py`). Pending commit hash on this slice.
