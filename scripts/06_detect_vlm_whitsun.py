...
Uses Claude Sonnet 4.6 via the Anthropic backend.  961 tiles (31x31) over the
17602x17602 Umbra GEC scene of Whitsun Reef.  Estimated cost ~$13.50 upper
bound (measured rate $0.014/tile).  Expected wall time ~2-3 hours.
...
Outputs (diagnostic, not committed):
    data/processed/vlm_detections/whitsun_20231206_position.parquet
    day0/scratch/whitsun_20231206_vlm_progress.jsonl
    day0/scratch/whitsun_20231206_vlm_summary.json
    day0/scratch/whitsun_20231206_vlm_detections.png
"""
...
# Auto-approval gates
GATE_TILE_COUNT = (800, 1200)
GATE_EST_COST = (10.0, 20.0)
...
PROGRESS_LOG = SCRATCH / "whitsun_20231206_vlm_progress.jsonl"
SUMMARY_JSON = SCRATCH / "whitsun_20231206_vlm_summary.json"
PREVIEW_PNG = SCRATCH / "whitsun_20231206_vlm_detections.png"
...
def main() -> None:
    ...
    def progress_cb(tile_info, response, done, total):
        ...
        print(
            f"[tile {done:>4}/{total}] {status}  origin=({rec['origin_row']:>5},{rec['origin_col']:>5})  "
    ...
    try:
        observations = detect_vessels_in_scene(
            ...
            source_id="umbra-2023-12-06-whitsun-vlm",
            backend=backend,
    ...
    tmp_dir = OUT_DIR / "_tmp_whitsun_20231206"
