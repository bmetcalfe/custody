"""Tennent VLM run: 2023-08-13. UMBRA-06 at ~0.41 m/px (fifth scene in Tennent timeline)

Same AOI geometry + backend + pipeline config as scripts/05 / 07.

Outputs (uncommitted pending review):
    data/processed/vlm_detections/tennent_20230813_position.parquet
    day0/scratch/tennent_20230813_vlm_progress.jsonl
    day0/scratch/tennent_20230813_vlm_summary.json
    day0/scratch/tennent_20230813_vlm_detections.png
"""
...
SCENE_DIR = (
    ...
    / "645d903f-ced5-40d9-94ed-6e4e91977c97/2023-08-13-15-00-09_UMBRA-06"
)
GEC_PATH = SCENE_DIR / "2023-08-13-15-00-09_UMBRA-06_GEC.tif"
META_PATH = SCENE_DIR / "2023-08-13-15-00-09_UMBRA-06_METADATA.json"
...
PARQUET_PATH = OUT_DIR / "tennent_20230813_position.parquet"
PROGRESS_LOG = SCRATCH / "tennent_20230813_vlm_progress.jsonl"
SUMMARY_JSON = SCRATCH / "tennent_20230813_vlm_summary.json"
PREVIEW_PNG = SCRATCH / "tennent_20230813_vlm_detections.png"
...
def main() -> None:
    ...
    def progress_cb(tile_info, response, done, total):
        ...
        print(
            f"[tennent_20230813] [tile {done:>3}/{total}] {status}  origin=({rec['origin_row']:>5},{rec['origin_col']:>5})  "
    ...
    try:
        observations = detect_vessels_in_scene(
            ...
            source_id="umbra-2023-08-13-vlm",
            backend=backend,
    ...
    # Handles empty-observations case (index_observations doesn't create a file
    # when the obs list is empty; rename would then crash).
    if PARQUET_PATH.exists():
        PARQUET_PATH.unlink()
    tmp_dir = OUT_DIR / "_tmp_tennent_20230813"
    ...
    summary = {
        ...
        "parquet_path": (
            str(PARQUET_PATH.relative_to(REPO_ROOT).as_posix())
