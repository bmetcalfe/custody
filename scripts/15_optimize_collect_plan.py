"""Constrained collection-plan optimizer CLI (ADR-0021 Slice 11).

Composes the full hypothesis-layer pipeline (Slices 3-10) and runs the
Slice 11 optimizer over candidate collect TYPES.  Reports both the
exhaustive and greedy baselines plus a recommended plan, under simple
budget / max-collects / required / excluded constraints.

Outcome weights and planning utility are deterministic heuristic proxies,
not calibrated probabilities and not a financial model.  Recommendations
name candidate collect types only; specific platform selection and
scheduling are downstream concerns.

The synthetic narrative builders are duplicated privately from
``scripts/14_counterfactual_collects.py`` (which duplicated from 13)
so the three scripts stay independently runnable without cross-script
imports.

Run::

    python scripts/15_optimize_collect_plan.py                                  # both
    python scripts/15_optimize_collect_plan.py --scenario tennent
    python scripts/15_optimize_collect_plan.py --scenario whitsun
    python scripts/15_optimize_collect_plan.py --scenario tennent --budget 0.5 --max-collects 1
    python scripts/15_optimize_collect_plan.py --scenario tennent --strategy greedy
"""
...
# ---------------------------------------------------------------------------
# Synthetic fixture builders (duplicated from scripts/14; private here).
# ---------------------------------------------------------------------------
...
def _run_scenario(
    ...
    *,
    ...
    if strategy == "exhaustive":
        # Re-render with the exhaustive plan as the recommended block.
        from custody.hypotheses.optimizer import PlanOptimizationReport
        forced = PlanOptimizationReport(
            scenario_id=report.scenario_id,
            exhaustive_plan=report.exhaustive_plan,
            greedy_plan=report.greedy_plan,
            recommended_plan=report.exhaustive_plan,
            comparison_summary=(
                f"strategy override: showing exhaustive plan "
                f"(utility {report.exhaustive_plan.total_value:.2f})"
            ),
            all_candidates=report.all_candidates,
        )
        text = format_optimizer_text(forced)
        ...
        forced = PlanOptimizationReport(
            ...
            exhaustive_plan=report.exhaustive_plan,
            greedy_plan=report.greedy_plan,
            recommended_plan=report.greedy_plan,
            comparison_summary=(
                f"strategy override: showing greedy plan "
                f"(utility {report.greedy_plan.total_value:.2f})"
            ),
            all_candidates=report.all_candidates,
        )
        text = format_optimizer_text(forced)
...
def main(argv: list[str] | None = None, *, out: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Constrained collection-plan optimizer over candidate collect "
            "TYPES (deterministic exhaustive + greedy baselines)."
        ),
    )
    parser.add_argument(
        "--scenario",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Total cost budget for the plan (default: 1.0).",
    )
    parser.add_argument(
        "--max-collects",
        dest="max_collects",
        type=int,
        default=2,
        help="Maximum number of collects in the plan (default: 2).",
    )
    parser.add_argument(
        "--strategy",
        choices=("recommended", "exhaustive", "greedy"),
        default="recommended",
        help="Which baseline to surface in the recommended block.",
    )
    ...
    for i, s in enumerate(scenarios):
        if i > 0:
            sink.write("\n")
        _run_scenario(
            sink, s, constraint=constraint, strategy=args.strategy,
        )
