"""Planner work queue CLI (ADR-0021 Slice 14).

Composes the full hypothesis-layer pipeline (Slices 3-13) for the
requested scenarios, optionally consults a Slice 13 review ledger for
the latest operator action per scenario, and prints a ranked work queue
explaining "which scenario needs attention first, why, and what should
the operator do next".

This CLI is a **decision-support interface only** — no live tasking, no
sensor commands.  Synthetic narrative builders are duplicated privately
from ``scripts/17_review_decision_packet.py`` so the eight CLIs stay
independently runnable.

Run::

    python scripts/18_planner_queue.py --scenario both
    python scripts/18_planner_queue.py --scenario tennent --format json
    python scripts/18_planner_queue.py --scenario both --review-ledger reviews.jsonl
"""
...
# ---------------------------------------------------------------------------
# Synthetic fixture builders (duplicated from scripts/17; private here).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Pipeline + queue assembly
# ---------------------------------------------------------------------------


def _scenario_pipeline(scenario_id: str):
    ...
    return state, health, rec, mv, cf, opt, policy
...
def main(argv: list[str] | None = None, *, out: TextIO | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Planner work queue: per-scenario decision-support items "
            "ranked by custody health, ambiguity, mission-value proxy, "
            "planning utility, and optional human review status. "
            "Decision support only; no live tasking or sensor commands."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--review-ledger", dest="review_ledger", default=None,
        help="Optional JSONL ledger (Slice 13 format) for per-scenario review status.",
    )
    parser.add_argument(
    ...
    review_records = ()
    if args.review_ledger is not None:
        review_records = load_review_ledger_jsonl(args.review_ledger)
    ...
    for s in scenarios:
        state, health, rec, mv, cf, opt, _ = _scenario_pipeline(s)
        if state.timestamp is not None:
            state_timestamps.append(state.timestamp)
        review = select_latest_review_for_scenario(review_records, s)
        items.append(create_queue_item(
            scenario_id=s, health=health, recommendation=rec,
            mission_value_report=mv, counterfactual_report=cf,
            optimization_report=opt, review_record=review,
        ))

    # Deterministic generated_at: max of state timestamps so the CLI output
    # is stable across runs without consulting wall-clock.
    if state_timestamps:
        generated_at = max(state_timestamps)
        if generated_at.tzinfo is None:
            generated_at = generated_at.replace(tzinfo=timezone.utc)
    else:
        generated_at = datetime(2024, 1, 1, tzinfo=timezone.utc)

    queue = build_planner_queue(items, generated_at=generated_at)

    if args.format == "json":
        sink.write(format_queue_json(queue))
    elif args.format == "md":
        sink.write(format_queue_markdown(queue))
    else:
        sink.write(format_queue_text(queue))
