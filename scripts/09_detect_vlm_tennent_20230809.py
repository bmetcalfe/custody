"""Driver for Tennent 2023-08-09 VLM detection run (held back from canonical scene dataset).

The 2023-08-09 UMBRA-06 acquisition is excluded from canonical VLM detection data
per ADR-0017 / data/processed/vlm_detections/README.md.  Post-hoc diagnosis
(day0/scratch/tennent_20230809_diagnosis.md) confirmed H2 (low-SNR acquisition):
scene-wide dynamic range is ~3x narrower than 07-02 (std 4.3 vs 11.5; p99 58 vs 102),
producing featureless speckle across the correctly-projected AOI.  The detection
pipeline correctly returned zero observations; the scene itself carries no signal.

This driver is retained for reproducibility but not expected to produce useful
observations.  Use detection/quality.py's assess_scene_quality() to screen
similar low-SNR scenes before VLM dispatch going forward.

Same AOI geometry + backend + pipeline config as scripts/05 / 07.

Outputs (uncommitted pending review):
    data/processed/vlm_detections/tennent_20230809_position.parquet
    day0/scratch/tennent_20230809_vlm_progress.jsonl
    day0/scratch/tennent_20230809_vlm_summary.json
    day0/scratch/tennent_20230809_vlm_detections.png
"""
...
GEC_PATH = SCENE_DIR / "2023-08-09-02-23-25_UMBRA-06_GEC.tif"
META_PATH = SCENE_DIR / "2023-08-09-02-23-25_UMBRA-06_METADATA.json"
...
PARQUET_PATH = OUT_DIR / "tennent_20230809_position.parquet"
PROGRESS_LOG = SCRATCH / "tennent_20230809_vlm_progress.jsonl"
SUMMARY_JSON = SCRATCH / "tennent_20230809_vlm_summary.json"
PREVIEW_PNG = SCRATCH / "tennent_20230809_vlm_detections.png"
...
def main() -> None:
    ...
    def progress_cb(tile_info, response, done, total):
        ...
        print(
            f"[tennent_20230809] [tile {done:>3}/{total}] {status}  origin=({rec['origin_row']:>5},{rec['origin_col']:>5})  "
    ...
    try:
        observations = detect_vessels_in_scene(
            ...
            source_id="umbra-2023-08-09-vlm",
            backend=backend,
    ...
    if PARQUET_PATH.exists():
        PARQUET_PATH.unlink()
    tmp_dir = OUT_DIR / "_tmp_tennent_20230809"
